...
\begin{enumerate}[label=(\roman{*})]
	...
	\item Seien $X_1$ und $X_2$ unabhängige poissonverteilte Zufallsvariablen mit $X_1\sim Poi(\lambda _1)$ und $X_2\sim Poi(\lambda _2)$, dann gilt für $X := X_1 + X_2 $:
		...
		\end{eqnarray*}
		Das heißt die Summer von poissonverteilten Zufallsvariablen ist wieder poissonverteilt.
	\end{enumerate}
...
\begin{lemmas} Die Poisson-Verteilung ist die Grenzverteilung der Binomial-Verteilung ($B_{n,p}(x)$ mit einer geringen Erfolgswahrscheinlichkeit $p\rightarrow 0$ und dafür einer hohen Anzahl an Versuchen $n\rightarrow \infty$, wenn gilt das $\lambda = np$ weder $0$ noch $\infty$ ist. 
\end{lemmas}

\textbf{Beweis:}
	Wir zeigen, dass der Grenzwert $n\rightarrow \infty$ einer binomialverteilten Zufallsvariable an der Stelle x, gegen den Wert einer poissonverteilten Zufallsvariablen an der Stelle x geht.
	\begin{eqnarray*}
		\lim_{x\rightarrow \infty} \mathbb(P)(X=x) &=& \lim_{x\rightarrow \infty} \frac{n!}{x!(n-x)!} \frac{\lambda}{n}^x \left(1-\frac{\lambda}{n}\right)^{n-x} \\
		&=& \lim_{x\rightarrow \infty} \frac{\lambda ^x}{x!} \frac{n(n-1)(n-2)...(n-x+1)}{n^x} \left( 1-\frac{\lambda}{n}\right)^{n} \left(1-\frac{\lambda}{n}\right)^{-x} \\
...
\end{eqnarray*}
ein stochastischer Prozess und wird als \textbf{Zähl- bzw. Erneuerungsprozess} bezeichnet. 
\end{defini}

Prozesse dieser Art werden z.B. in der Systemtheorie eingesetzt um die Ausfälle einer Komponente in einem bestimmten Zeitraum zu zählen. Deshalb werden die $T_n$ häufig als \textbf{Zwischenankunftszeiten} bezeichnet.
...
TODO:
Eigenschaften der Markov-Kette charakterisieren
Pfade durch Potenzen der Übergangsmatrix
Irreduzibilität
