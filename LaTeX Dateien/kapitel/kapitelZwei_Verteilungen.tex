...
\begin{lemmas} \label{lem:expLemma} Sei X eine absolut stetige postive Zufallsvariable, dann gilt  $X \sim exp(\lambda)$ genau dann wenn für alle $x, t > 0$ gilt, dass
...
\begin{folg}
Die Exponentialverteilung ist ein Spezialfall der Gamma-Verteilung: Sei $X \sim \gamma(\alpha, r)$ dann gilt für $r =1$ gilt $X \sim exp(\alpha)$. Da für x>0 gilt:
	\begin{eqnarray*}
		\gamma_{\alpha, 1}(x) = \frac{\alpha ^1}{\Gamma(1)}x^{0} e^{-\alpha x} = \frac{\alpha}{\int_0^{\infty} e^{-y} dy } e^{-\alpha x} = \alpha e^{-\alpha x}
	\end{eqnarray*}
und das ist die Dichte einer zum Parameter $\alpha$ exponentialverteilten Zufallsgröße.
\end{folg}
...
Es gilt:  
	\begin{eqnarray*}
		\mathbb{P}(X_1+X_2 \leq x) &=& \int_0^x \gamma_{\alpha, r_1}(t)\gamma_{\alpha, r_2}(x-t)dt \\
															 ...
															 &=& \gamma_{\alpha, r_1 +r2}(t) \frac{\Gamma(r_1+r_2)}{\Gamma(r_1)\Gamma(r_2)} \underbrace{\int_0^x t^{r_1-1}(s-t)^{r_2-1}dt}_{u=\frac{s}{t}} \\
 &=& \gamma_{\alpha, r_1 +r2}(t) \frac{\Gamma(r_1+r_2)}{\Gamma(r_1)\Gamma(r_2)} \underbrace{\int_0^1 u^{r_1-1}(1-u)^{r_2-1}du}_{Eulersche Beta-Integral} \\
	...
	\end{eqnarray*}
Der zweite Teil der Aussage folgt direkt aus dieser Beziehung.
\qed