...
Wir schätzen $\lambda$, auf Basis relative Häufigkeit eines Unfalls im betrachteten Zeitraum\footnote{i.d.R. ein Kalenderjahr}. Im ersten Schritt betrachten wir diese unabhängig von Alter und Geschlecht, d.h. sie ist für den gesamten Beobachtungszeitraum konstant. Dann gilt für die Wartezeiten $\{T_k\}_{k \geq 0}$, dass die Zeit bis zum nächsten Unfall unabhängig davon ist, wie viel Zeit bereits ohne Unfall verstrichen ist. Sei $F(x)$ die Verteilung der Wartezeiten, dann gilt also: 
...
Damit folgt gemäß Lemma \ref{expLemma}, dass $F(x)\sim exp(\lambda)$ und $N_{\lambda} = \{N_t\}_{t>0}$ ist gemäß Definition \ref{poiPro} ein Poisson-Prozess mit Parameter $\lambda $. Damit folgt aufgrund von \ref{lem:wartezeit}, dass der Zeitpunkt zu dem die Beobachtung beginnt keinen Einfluss auf den Zeitpunkt des ersten beobachteten Unfalls hat. Wie bereits am Anfang erwähnt, ist die Unfallwahrscheinlichkeit von verschiedenen Faktoren abhängen und dabei in erster Linie natürlich vom Unfalltyp. Deshalb zerlegen wir den Prozess in mehrere Teilprozesse:\\
...
Zusammenfassend lässt sich festhalten, dass sich der Poisson-Prozess direkt aus den getroffen Modellannahmen ableiten lässt und die Möglichkeit bietet auf einfache Weise verschiedene Modelle für einzelne Unfalltypen zu erstellen und zu kombinieren.
