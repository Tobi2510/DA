...
Als nächstes werden wir eine weitere wichtige Klasse an stochastischen Prozessen vorstellen. Dieser Abschnitt basiert zum Großteil auf dem Skript von Professor Wolfgang König, "`Wahrscheinlichkeitstheorie I und II"'\footnote{Aktuelle Version unter \url{http://www.wias-berlin.de/people/koenig/www/Skripte.html} (Stand: 16.01.2015)}. Für den gesamten Abschnitt gilt, dass $I$ eine nichtleere, endliche oder höchstens abzählbar unendliche Menge ist.
...
\textbf{Beweis:}
Der Beweis das die Gleichung \eqref{eq:markov1} für eine Markov-Kette gilt, erfolgt leicht mithilfe von vollständiger Induktion nach n zusammen mit der Definition der Übergangswahrscheinlichkeiten: \\
...
\begin{satz} Sei $P$ irreduzibel und rekurrent, und sei $k \in I$. Weiterhin sei $\gamma_k$ ein Maß, das für eine, in einem beliebigen Zustand $k\in I$ gestartete Markov-Kette, die erwartete Anzahl an Besuchen in einem Zustand $i \in I$ bis zur ersten Rückkehr der Kette nach $k$ zählt. Das heißt es ist
\begin{eqnarray*}
...
\begin{itemize}
\item $(i)$:
Durch Anwendung des Satzes der monotonen Konvergenz\footnote{siehe Anhang } und Ausnutzen der Markov-Eigenschaft, können wir zeigen, dass $\gamma_k$ ist ein invariantes Maß. Dazu sei $i\in I$ ein beliebiger Zustant, dann gilt
\begin{eqnarray*}
...
\begin{eqnarray*}
	\lambda(j) &=& \sum_{i\in I\backslash\{k\}} \lambda(i) p_{i,j} + p_{k,j}\\
						...
						&\geq& \mathbb{P}(T_{k,k} \geq r, X_r =j)\\
...
\begin{satz} \label{sa:gleichgewicht} Sei $\{X_n, n \in \mathbb{N}\}$ eine irreduzible Markov-Kette mit Übergangsmatrix $P$, dann sind folgende Aussagen äquivalent: 
	...
	\end{enumerate}
	Außerdem gilt falls eine dieser Bedingungen erfüllt, dass die Gleichgewichtsverteilung $\pi$ eindeutig bestimmt ist und gilt $\pi(i) = \frac{1}{\mu _i}$.
\end{satz}
...
Wir zeigen, dass das im letzten Satz defininierte invariante Maß $\gamma$ auch zu einer Verteilung normiert werden kann und damit dann eine Gleichgewichtsverteilung ist. Dazu sei im folgenden $k \in I$ der positiv rekurrente Zustand. Aufgrund der Irreduzibilität ist damit auch die Markov-Kette rekurrent und es gilt  
\begin{eqnarray*}
...
\begin{satz} Sei $\{X_n, n\mathbb{N}\}$ eine irreduzible Markov-Kette mit aperiodischen und positiv rekurrenten Zuständen und Gleichgewichtsverteilung $\pi$. Dann gilt für alle $i, j \in I$ 
\begin{eqnarray*}
